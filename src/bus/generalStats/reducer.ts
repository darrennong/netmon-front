...
export const generalStatsReducer = (state = initialState, { type, payload }:event) => {
  switch (type) {
    ...
    case types.BLOCK_CHART_UPDATE:
      return {
        ...state,
        blockChart: [
          ...payload.reverse().slice(0, TRANSACTIONS_LIMIT),
          ...(payload.length > TRANSACTIONS_LIMIT
            ? []
            : state.blockChart.slice(0, TRANSACTIONS_LIMIT - payload.length)),
        ],
      };
